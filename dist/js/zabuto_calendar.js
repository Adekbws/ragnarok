...
$.fn.zabuto_calendar = function (options) {
    ...
    this.each(function () {
        ...
        function appendDaysOfMonth($calendarElement, $tableObj, year, month) {
            ...
            //custom print a few days a next month
            var nextMonthDay=1;

            //custom print a few days a prev month
            var lastDayinPrevMonth=31;
            if(month!=0)
                lastDayinPrevMonth= calcLastDayInMonth(year, month-1);
            var startDayinPrevMonth=lastDayinPrevMonth-firstDow+1;
                ...
                for (var dow = 0; dow < 7; dow++) {
                    ...
                    if (dow < firstDow || currDayOfMonth > lastDayinMonth) {
                         if(currDayOfMonth > lastDayinMonth)
                        {
                            ...
                            $dowRow.append(printDay($calendarElement, $tableObj, yearNumber, numberMonth, nextMonthDay, true));
                            nextMonthDay++;
                         ...
                         if(dow < firstDow)
                         {
                            ...
                            $dowRow.append(printDay($calendarElement, $tableObj, yearNumber, numberMonth, startDayinPrevMonth, true));
                            startDayinPrevMonth++;
                         }
                    } 
                    else {
                      
                       $dowRow.append(printDay($calendarElement, $tableObj, year, month, currDayOfMonth, false));
                        currDayOfMonth++;
                    }
                    if (dow == 6) {
                        firstDow = 0;
        ...
        function printDay($calendarElement, $tableObj, year,monthNo,dayNo,otherMonth)
        {
            var dateId = $calendarElement.attr('id') + '_' + dateAsString(year, monthNo, dayNo);
            var dayId = dateId + '_day';

            var $dayElement = $('<div id="' + dayId + '" class="day" >' + dayNo + '</div>');
            $dayElement.data('day', dayNo);

            if (isToday(year, monthNo, dayNo)) 
            {
                $dayElement.addClass('today');
                if ($calendarElement.data('showToday') === true) 
                {
                    $dayElement.html('<span class="badge badge-today">' + dayNo + '</span>');
                }
            }

            if(otherMonth==true)
                var $dowElement = $('<td id="' + dateId + '" class="otherMonth"></td>');
            else
                var $dowElement = $('<td id="' + dateId + '"></td>');
            
            $dowElement.append($dayElement);
            $dowElement.data('date', dateAsString(year, monthNo, dayNo));
